...
case $1 in
    ...
    --web)
        echo "New Submissions in WebDev : "
        if [ $oldstat = 1 ]
        then 
            while IFS=" " read -ra searchtxt
            do
                

                old="`sed -rn "s/${searchtxt[0]}.*WebDev:([^ ]*).*/\1/p" /home/core/.displayStatus_old.txt`"
                new="`sed -rn "s/${searchtxt[0]}.*WebDev:([^ ]*).*/\1/p" /home/core/displayStatus.txt`" 
                if [[ $old != $new ]] || [[ -z $old ]]
                then 
                    echo ${searchtxt[0]}
                fi
            ...
            echo "`sed -rn "s/([0-9]+).*WebDev.*/\1/p" /home/core/displayStatus.txt`"
        fi  ;;

    --app)
        echo "New Submissions in AppDev : "
        if [ $oldstat = 1 ]
        then 
            while IFS=" " read -ra searchtxt
            do
                old="`sed -rn "s/${searchtxt[0]}.*AppDev:([^ ]*).*/\1/p" /home/core/.displayStatus_old.txt`"
                new="`sed -rn "s/${searchtxt[0]}.*AppDev:([^ ]*).*/\1/p" /home/core/displayStatus.txt`" 
                if [[ $old != $new ]] || [[ -z $old ]]
                then 
                    echo ${searchtxt[0]}
                fi
            ...
            echo "`sed -rn "s/([0-9]+).*AppDev.*/\1/p" /home/core/displayStatus.txt`"
        fi  ;;
    
    --sysad)
        echo "New Submissions in SysAd : "
        if [ $oldstat = 1 ]
        then 
            while IFS=" " read -ra searchtxt
            do
                ...
                old="`sed -rn "s/${searchtxt[0]}.*SysAd:([^ ]*).*/\1/p" /home/core/.displayStatus_old.txt`"
                new="`sed -rn "s/${searchtxt[0]}.*SysAd:([^ ]*).*/\1/p" /home/core/displayStatus.txt`" 
                if [[ $old != $new ]] || [[ -z $old ]]
                then 
                    echo ${searchtxt[0]}
                fi
            ...
            echo "`sed -rn "s/([0-9]+).*SysAd.*/\1/p" /home/core/displayStatus.txt`"
        fi  ;;
        
    *)
        echo "Usage : displayStatus -a           -> Print out the total percentage of people who submitted each task with list of mentees who submitted the task since the last time the core used this alias
